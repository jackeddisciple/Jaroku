...
// exists here for the assignment at agent creation and in `client/src/lib/emojiPalette.ts` for the
// picker. `test:agent-emoji` reads the other file's SOURCE and fails on any difference, which is
// the same arrangement `test:egress-connectors` uses to hold the two private-range block lists to
// each other across TypeScript and Python.
//
