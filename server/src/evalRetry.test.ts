...
// --- 2. a retry must not lose the failed attempt's spend ------------------------------
{
  ...
  const ds = evalStore.createDataset("agent-r", "retries");
  ...
  const [job] = evalStore.createJobs(run.id, [
    { example_id: ex.id, provider: "anthropic", model: "claude-haiku-4-5" },
  ]);

  // Attempt 1 burns tokens, then rate-limits.
  evalStore.finishJob(job!.id, "failed", { cost_usd: 0.03, error: "rate limit" });
  evalStore.retryJob(job!.id, 1, new Date(Date.now() + 1000));
  // Attempt 2 succeeds, more cheaply.
  evalStore.finishJob(job!.id, "succeeded", { cost_usd: 0.02, tokens: 900, latency_ms: 800 });

  const after = evalStore.getJob(job!.id)!;
  ...
  check("trueSpend uses the cumulative column",
    Math.abs(evalStore.trueSpend(run.id) - 0.05) < 1e-9, `got ${evalStore.trueSpend(run.id)}`);
...
// --- 3. the ceiling is checked against true spend, not the comparison figure ----------
{
  ...
  const ds = evalStore.createDataset("agent-b", "budget");
  const ex = evalStore.addExample(ds.id, "hello");
  const rubric = evalStore.putRubric({ dataset_id: null, name: "r", criteria: [] });
  const run = evalStore.createEvalRun({
  ...
  const jobs = evalStore.createJobs(run.id, Array.from({ length: 5 }, () => ({
    example_id: ex.id, provider: "anthropic", model: "claude-haiku-4-5",
  })));
  ...
  evalStore.finishJob(jobs[0]!.id, "failed", { cost_usd: 0.05, error: "rate limit" });
  evalStore.finishJob(jobs[1]!.id, "failed", { cost_usd: 0.06, error: "rate limit" });
  ...
  check("true spend sees all of it",
    Math.abs(evalStore.trueSpend(run.id) - 0.11) < 1e-9, `got ${evalStore.trueSpend(run.id)}`);
  check("true spend has crossed the ceiling the comparison figure would have missed",
    evalStore.trueSpend(run.id) >= 0.10 && (agg.providers[0]!.comparisonCostUsd ?? 0) < 0.10);

  // The abort path: queued work is cancelled so nothing further is started.
  const cancelled = evalStore.cancelQueuedJobs(run.id, "budget ceiling reached");
  evalStore.setEvalStatus(run.id, "aborted_over_budget", "over budget");
  check("remaining jobs cancelled on abort", cancelled === 3, `cancelled ${cancelled}`);
  check("no job left queued", evalStore.jobsForEval(run.id).every((j) => j.status !== "queued"));
  check("eval records WHY it stopped",
    evalStore.getEvalRun(run.id)!.status === "aborted_over_budget");
  const spendAfter = evalStore.trueSpend(run.id);
