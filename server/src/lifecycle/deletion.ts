...
const DELETION_ORDER = [
  ...
  // The items before the threads they hang off, and both before `agents`. That last part is the one
  // place this list disagrees with the rest of the schema on purpose: everywhere else a child goes
  // first because its cascade would take it anyway, but a thread's foreign key is ON DELETE SET
  // NULL, so deleting `agents` first would leave every thread standing with a nulled agent and a
  // freshly written snapshot — rows that are then deleted a line later, having been pointlessly
  // rewritten. One statement instead of two, and the receipt's count is the same either way.
  "thread_items",
  "threads",
  "agent_versions",
