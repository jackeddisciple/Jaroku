...
--
-- THE POSTGRES HALF IS THE SAME TWO STATEMENTS. `ADD COLUMN` of a nullable column with no default
-- is a catalogue-only change here — no table rewrite, no lock held past the statement — which is
-- the other half of why this column is nullable: the NOT NULL form would rewrite `agents` under an
-- ACCESS EXCLUSIVE lock as well as breaking the version still serving.
