        ...
        {server.tools.length === 0 ? (
          <p className="px-2 py-6 text-center text-[12px] text-muted">
            {server.status === "connected"
              ...
              : "No tools discovered yet — connect successfully to see what this server offers."}
          </p>
        ) : (
          <>
    ...
    <div className="flex h-full flex-col overflow-hidden">
      ...
      {servers.length === 0 ? (
        <div className="flex flex-1 items-center justify-center px-6">
          <p className="max-w-sm text-center text-[12px] leading-[1.6] text-muted">
            No MCP servers connected.
            <span className="mt-1 block text-faint">
              Connecting one discovers the tools it offers so you can give an agent the
              specific ones it needs.
            </span>
          </p>
        </div>
      ) : selected ? (
